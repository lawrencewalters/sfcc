...
if [[ "$master_catalog_id" =~ "club" ]]; then
    echo -e "\n${CYAN}Processing recommendations for club catalog ${YELLOW}$master_catalog_id${CYAN} and site catalog ${YELLOW}$site_catalog_id${NC}"
    #### Clubs with recommendations handling ####
    # get a subset of the recommendations for the products in our new trimmed catalog
    echo -e "\n   ${CYAN}get static rec target products into ${YELLOW}$TMP_DIR/$dir_name/recs-targets.txt${CYAN} from our new trimmed catalog${GRAY}"
    ...
    echo -e "\n   ${CYAN}rec target products master catalog to ${YELLOW}$TMP_DIR/$dir_name/catalogs/$master_catalog_id-recs-targets/catalog.xml${GRAY}"
    ...
    echo -e "\n   ${CYAN}merging main master catalog with rec targets catalog${GRAY}"
    ...
    echo -e "\n   ${CYAN}updating variations.txt to include all products from merged catalog${GRAY}"
    echo -n "productIds=" > "$TMP_DIR/$dir_name/variations.txt"
    grep -oP '(?<=product-id=")[^"]+' "$TMP_DIR/$dir_name/catalogs/$master_catalog_id/catalog.xml" | sort | uniq | awk '{print "\"" $0 "\""}' | paste -sd'|' - >> "$TMP_DIR/$dir_name/variations.txt"

    # rec target products real pricing using updated variations.txt (now includes all products)
    echo -e "\n   ${CYAN}generating pricing for all products (including rec targets) to ${YELLOW}$TMP_DIR/$dir_name/pricebooks/$pricebook_id.xml${GRAY}"
    cat "$TMP_DIR/$dir_name/variations.txt" | xargs -t java -jar "$SAXON_JAR" -s:"$TMP_DIR/$SOURCE_DIR/pricebooks/$pricebook_id.xml" -xsl:"$SCRIPT_DIR/generate-pricebook-from-variations.xslt" pricebookId="$pricebook_id" currency="$currency" pricebookParentId="" > "$TMP_DIR/$dir_name/pricebooks/$pricebook_id.xml"

    # rec target products - site catalog assignments
    echo -e "\n   ${CYAN}rec target site catalog to ${YELLOW}$TMP_DIR/$dir_name/catalogs/$site_catalog_id-recs-targets/catalog.xml${GRAY}"
    ...
    echo -e "\n   ${CYAN}merging main site catalog with rec targets site catalog${GRAY}"
