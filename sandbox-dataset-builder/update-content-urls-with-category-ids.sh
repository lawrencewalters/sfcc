...
# where [input_file] is the file that contains export of the content library you want transformed
#
# build the mapping file from .....

input_file="$1"

# Loop through the mapping file
while IFS=":" read -r url category_id; do
  if [[ -n "$url" && -n "$category_id" ]]; then
    # Use sed to replace the page-url with the category-id
    sed -i "s|\"/*$url/*\"|\"\$url('Search-Show','cgid','$category_id')$\"|g" "$input_file"
  fi
done < data/url-to-category-id-mapping.txt 